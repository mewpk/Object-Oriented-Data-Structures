...
#  (ทุกชิ้นที่ i ที่ a < i < b) จะต้องอยู่ในกล่องนี้ด้วย 
...
def is_feasible(weights, k, max_weight):
    current_weight = 0
    boxes = 0
    ...
    return boxes + 1 <= k  # Check if the number of boxes used is less than or equal to k

def minimum_weight_for_boxes(weights, k):
    left = max(weights)
    right = sum(weights)
    while left < right:
        mid = (left + right) // 2
        if is_feasible(weights, k, mid):
            right = mid
        else:
            left = mid + 1
    return left

weight , box  = input("Enter Input : ").split("/")
weight ,  box  = list(map(int,weight.split(" "))) , int(box)
result = minimum_weight_for_boxes(weight, box)
print(f"Minimum weigth for {box} box(es) = {result}")
